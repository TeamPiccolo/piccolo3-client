...
class PiccoloControl(PiccoloClientComponent):
    ...
    async def record_sequence(self, run=None, nsequence=None,
                              auto=None, delay=None, target=None,
                              at_time=None, interval=None, end_time=None):
        """start recording a batch

        :param run: name of the current run
        :param nsequence: the number of squences to record
        :param auto: can be -1 for never; 0 once at the beginning;
                     otherwise every nth measurement
        ...
        await self.a_put('record_sequence', run=run, nsequence=nsequence,
                         auto=auto, delay=delay, target=target,
                         at_time=at_time, interval=interval,
                         end_time=end_time)
...
async def main():
    ...
    await p.record_sequence('test_client',
                            nsequence=10,
                            at_time=now + datetime.timedelta(seconds=15))
